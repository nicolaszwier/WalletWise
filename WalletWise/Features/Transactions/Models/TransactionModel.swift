...
class TransactionModel {
    ...
    func saveMany(transactions: [Transaction]) async throws -> DefaultResponse {
        
        guard var request = try? HttpService().buildUrlRequest(method: "POST", endpoint: Constants.ApiConstants.Transactions.postManyTransactions, params: []) else {
            throw NetworkError.invalidURL
        }
        
        print(transactions)
        
        request.httpBody = try? HttpService().customEncoder().encode(transactions)
